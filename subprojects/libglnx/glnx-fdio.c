...
static gboolean
glnx_name_to_handle_at_internal (int                  dfd,
                                 const char          *path,
                                 int                  flags,
  ...
  for (;;)
    {
      ...
      r = name_to_handle_at (dfd, path,
                             handle,
                             /* the kernel has no problem with bigger buffers */
                             (int *) &mnt_id,
                             flags);

      if (r < 0 &&
          (errno != EOVERFLOW || handle->handle_bytes <= handle_bytes))
 ...
 * @flags: Flags for name_to_handle_at()
                        ...
                        GError             **error)
{
  ...
  g_return_val_if_fail ((flags & ~(AT_SYMLINK_FOLLOW |
                                   ...
                                   AT_HANDLE_FID |
                                   AT_HANDLE_MNT_ID_UNIQUE |
                                   AT_HANDLE_CONNECTABLE)) == 0, FALSE);

  if (glnx_name_to_handle_at_internal (dfd, path,
                                       flags,
...

  {
    ...
    if (!glnx_statx (dfd, path, statx_flags, GLNX_STATX_MNT_ID_UNIQUE, &stx, error))
      return FALSE;
