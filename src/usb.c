...
G_DEFINE_AUTOPTR_CLEANUP_FUNC (XdpDbusUsbSkeleton, g_object_unref)
                              ...
                              GUdevDevice   *device)
{
  ...
  if (vendor_id_str != NULL && validate_hex_uint16 (vendor_id_str, 2, &device_vendor_id))
    device_has_vendor_id = TRUE;

  product_id_str = g_udev_device_get_property (device, "ID_MODEL_ID");
  if (product_id_str != NULL && validate_hex_uint16 (product_id_str, 2, &device_product_id))
                     ...
                     GError          **error)
{
  ...
  session = g_initable_new (xdp_usb_session_get_type (),
                            NULL, error,
