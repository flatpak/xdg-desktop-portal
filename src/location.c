                       ...
                       GVariant *arg_options)
{
  ...
  if (!xdp_session_export ((XdpSession *)session, &error))
    {
       g_warning ("Failed to export session: %s", error->message);
       xdp_session_close ((XdpSession *)session, FALSE);
    }
  else
    {
      g_debug ("CreateSession new session '%s'",  ((XdpSession *)session)->id);
      xdp_session_register ((XdpSession *)session);
    }

  xdp_dbus_location_complete_create_session (object, invocation, ((XdpSession *)session)->id);
