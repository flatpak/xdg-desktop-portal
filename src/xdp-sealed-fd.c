                              ...
                              GError **error)
{
  ...
  xdp_autofd int fd = memfd;
  int seals;

  g_return_val_if_fail (fd != -1, NULL);

  if ((seals = fcntl (fd, F_GET_SEALS)) == -1)
   {
      saved_errno = errno;
                              ...
                              GError **error)
{
  ...
  if ((fd = memfd_create ("sealed-fd-from-bytes", MFD_ALLOW_SEALING)) == -1)
    {
      int saved_errno;
  ...
  if ((shm = (char *) mmap (NULL, bytes_len, PROT_WRITE, MAP_SHARED, fd, 0)) == (void *) -1)
    {
      int saved_errno;
                               ...
                               GError      **error)
{
  ...
  if ((fd = g_unix_fd_list_get (fd_list, fd_id, error)) == -1)
    return NULL;
