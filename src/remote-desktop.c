...
gboolean
is_remote_desktop_session (XdpSession *session)
{
                       ...
                       GVariant *arg_options)
{
  ...
  session = (XdpSession *)remote_desktop_session_new (arg_options, request, &error);
  if (!session)
    {
      g_dbus_method_invocation_return_gerror (invocation, error);
      return G_DBUS_METHOD_INVOCATION_HANDLED;
...
{
  xdp_session_persistence_replace_restore_data_with_token ((XdpSession *) remote_desktop_session,
                                                           REMOTE_DESKTOP_TABLE,
                                                           in_out_results,
