...
static gboolean
is_screen_cast_session (XdpSession *session)
{
                       ...
                       GVariant *arg_options)
{
  ...
  session = (XdpSession *)screen_cast_session_new (arg_options, request, &error);
  if (!session)
    {
      g_dbus_method_invocation_return_gerror (invocation, error);
      return G_DBUS_METHOD_INVOCATION_HANDLED;
...
{
  xdp_session_persistence_replace_restore_data_with_token ((XdpSession *) screen_cast_session,
                                                           SCREEN_CAST_TABLE,
                                                           in_out_results,
