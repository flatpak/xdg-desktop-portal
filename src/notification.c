  ...
  while (*str)
    {
      ...
      while (g_unichar_isspace (c) && *str)
        {
          needs_space = composed->len > 0 && composed->str[composed->len - 1] != ' ';
                   ...
                   GError          **error)
{
  ...
  context = g_markup_parse_context_new (&markup_parser, G_MARKUP_DEFAULT_FLAGS, composed, NULL);
  ...
  if (g_markup_parse_context_parse (context, "<markup>", -1, error) &&
      ...
      g_markup_parse_context_end_parse (context, error))
    {
      g_variant_builder_add (builder, "{sv}",
                             "markup-body",
                             g_variant_new_take_string (g_strchomp (g_string_free_and_steal (g_steal_pointer (&composed)))));
                       ...
                       GError          **error)
{
  ...
  if (strcmp (key, "themed") == 0)
   ...
   {
     GVariant *file_icon = NULL;

     file_icon = convert_serialized_fd_to_serialized_file (value, app_info, fd_list, error);
     if (file_icon)
       {
          if (xdp_validate_serialized_icon (file_icon, FALSE, NULL, NULL))
            g_variant_builder_add (builder, "{sv}", "icon", file_icon);
                        ...
                        GError          **error)
{
  ...
  if (g_variant_is_of_type (sound, G_VARIANT_TYPE_STRING))
    {
      ...
      if (strcmp (key, "silent") == 0)
        {
          g_variant_builder_add_value (builder, sound);
          return TRUE;
        }
  ...
  if (strcmp (key, "bytes") == 0)
   ...
   {
     GVariant *file_sound = NULL;

     file_sound = convert_serialized_fd_to_serialized_file (value, app_info, fd_list, error);
     if (file_sound)
       {
         g_variant_builder_add (builder, "{sv}", "sound", file_sound);
         return TRUE;
       }
    }
  else
...
static gboolean
parse_content_type (GVariantBuilder  *builder,
                    GVariant         *value,
                    GError          **error)
{
  const char *content_type;
  const char *supported_content_types[] = {
    "im.message",
    "alarm.ringing",
    "call.incoming",
    "call.ongoing",
  ...
  if (!check_value_type ("content-type", value, G_VARIANT_TYPE_STRING, error))
    return FALSE;

  content_type = g_variant_get_string (value, NULL);

  if (!g_strv_contains (supported_content_types, content_type) && !g_str_has_prefix (content_type, "x-"))
    {
      g_set_error (error,
                   XDG_DESKTOP_PORTAL_ERROR,
                   XDG_DESKTOP_PORTAL_ERROR_INVALID_ARGUMENT,
                   "%s is not a supported content-type", content_type);
      return FALSE;
    }

  g_variant_builder_add (builder, "{sv}", "content-type", value);
