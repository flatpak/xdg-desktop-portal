...
static int
validate_sound (const char *filename)
{
  ...
  if (!uri)
    {
      g_printerr ("Invalid filename %s: %s\n", filename, error->message);
      return 1;
    }
  ...
  if (!discoverer)
    {
      g_printerr ("Failed to create gstreamer discoverer: %s\n", error->message);
      return 1;
    }
  ...
  switch (result) {
    case GST_DISCOVERER_URI_INVALID:
      g_printerr ("'%s' is invalid\n", filename);
      return 1;
    case GST_DISCOVERER_ERROR:
      g_printerr ("Couldn't discover media type of '%s': %s\n", filename, error->message);
      return 1;
    case GST_DISCOVERER_TIMEOUT:
      g_printerr ("Couldn't discover media type of '%s': Timeout\n", filename);
      return 1;
    case GST_DISCOVERER_BUSY:
      g_printerr ("Couldn't discover media type of '%s': Busy\n", filename);
      return 1;
    case GST_DISCOVERER_MISSING_PLUGINS:
      {
        ...
        g_printerr ("Couldn't discover media type of '%s': Missing plugins: %s\n",
                    filename,
                    str);
        return 1;
      }
  ...
  if (!stream_info)
    {
      g_printerr ("File '%s' doesn't contain a valid stream\n", filename);
      return 1;
    }
  ...
  if (stream_next)
    {
      g_printerr ("File '%s' is allowed to only contain a single stream\n", filename);
      return 1;
    }
  ...
  if (GST_IS_DISCOVERER_CONTAINER_INFO (stream_info))
    {
      ...
      if (!gst_caps_is_fixed (container_caps) || gst_caps_get_size (container_caps) != 1)
        {
          g_printerr ("Media format of '%s' is to complex\n", filename);
          return 1;
        }

      if (!gst_structure_has_name (structure, "audio/ogg"))
        {
          g_printerr ("'%s' contains a unsupported container format\n", filename);
          return 1;
        }
      ...
      if (streams->next)
        {
          g_printerr ("'%s' supported sound is allowed to only contain a single stream\n", uri);
          return 1;
        }
  ...
  if (format == NULL)
    {
      g_printerr ("'%s' contains a unsupported sound format\n", filename);
      return 1;
    }
...
rerun_in_sandbox (const char *filename)
{
  ...
  add_args (args, validate_sound, filename, NULL);
