      ...
      if (renamed_to == NULL)
      ...
      else /* renamed_to != NULL */
        {
          ...
          if (!g_key_file_save_to_file (new_key_file, desktop_path, &error))
            {
              g_warning ("Couldn't rewrite desktop file %s: %s", desktop_path, error->message);
              continue;
            }
          ...
          if (!g_file_set_display_name (desktop_file, new_desktop, NULL, &error))
            {
              g_warning ("Unable to rename desktop file %s -> %s: %s", desktop_name, new_desktop, error->message);
              continue;
            }
