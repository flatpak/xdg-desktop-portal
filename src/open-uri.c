                 ...
                 GVariant *arg_options)
{
  ...
  XdpDbusImplLockdown *lockdown = xdp_context_get_lockdown (open_uri->context);
  g_autoptr(GTask) task = NULL;
  gboolean writable;
  gboolean ask;
  const char *activation_token = NULL;
...
static OpenURI *
open_uri_new (XdpContext            *context,
              XdpDbusImplAppChooser *app_chooser_impl)
{
  OpenURI *open_uri;

  open_uri = g_object_new (open_uri_get_type (), NULL);
  open_uri->context = context;
...
init_open_uri (XdpContext *context)
{
  ...
  open_uri = open_uri_new (context, impl);
