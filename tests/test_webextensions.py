...
class TestWebExtensions:
    ...
    def create_session(self, dbus_con):
        ...
        options = {"mode": "mozilla", "session_handle_token": session_token}
        session = xdp.Session(
            dbus_con,
    ...
    def get_manifest(self, name, extension_or_origin, dbus_con):
        ...
        return webextensions_intf.GetManifest(
            self.session_handle, name, extension_or_origin
        )
    ...
    def get_pipes(self, dbus_con):
        webextensions_intf = xdp.get_portal_iface(dbus_con, "WebExtensions")
        response = webextensions_intf.GetPipes(self.session_handle, {})
        return response
    ...
    def test_web_extensions_basic(self, portals, app_id, dbus_con):
        ...
        assert (
            "some-extension@example.org" in manifest["allowed_extensions"]
    ...
    def test_web_extensions_denied(self, portals, app_id, dbus_con):
        ...
        try:
            ...
            assert (
                e.get_dbus_name() == "org.freedesktop.DBus.Error.AccessDenied"
            ), "unknown exception"
        assert pipes is None, "Expected pipes is None"
