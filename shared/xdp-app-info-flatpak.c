      ...
      if (errno == EACCES)
        {
          ...
          /* EACCES also happens when the target process is non-dumpable because it
           * was exec'd with file capabilities set (e.g. cap_net_admin on kdeconnectd,
           * cap_sys_nice on kwin_wayland) -- the kernel then denies /proc/<pid>/root
           * access to any other process, including same-uid ones, without
           * CAP_SYS_PTRACE. This is unrelated to sandboxing and common for ordinary
           * system daemons. We can't treat *every* EACCES this way though: a
           * genuinely Flatpak-confined process could make itself non-dumpable too,
           * via the unprivileged prctl(PR_SET_DUMPABLE, 0), purely to be
           * misidentified as unsandboxed here. So only take this path when the
           * target actually holds real capabilities -- something a Flatpak sandbox
           * (which unconditionally strips all capabilities from confined apps)
           * could never grant it, and the app itself can't fake.
           */
          if (xdp_pid_has_capabilities (pid))
            {
              g_set_error (error, XDP_APP_INFO_ERROR, XDP_APP_INFO_ERROR_WRONG_APP_KIND,
                           "Not a flatpak (capability-bearing process)");
              return -1;
            }
        }
