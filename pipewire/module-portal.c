...
static void
context_check_access(void                  *data,
...
static void
on_portal_pid_received(DBusPendingCall *pending,
                       void            *user_data)
{
...
static void
update_portal_pid(struct impl *impl)
{
  DBusMessage *m;
  ...
  if (!dbus_message_append_args (m,
                                 DBUS_TYPE_STRING,
                                 &(const char *){ PORTAL_SERVICE_NAME },
                                 DBUS_TYPE_INVALID))
    return;
...
static int
init_dbus_connection(struct impl *impl)
{
  DBusError error;
  ...
  dbus_connection_add_filter (impl->bus, name_owner_changed_handler, impl, NULL);
...
SPA_EXPORT int
pipewire__module_init(struct pw_impl_module *module,
                      const char            *args)
{
